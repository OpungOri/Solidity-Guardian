...
class Orchestrator:
    ...
    def run(self) -> list[dict[str, Any]]:
        ...
        from .detectors import detect_all, rule_ids
        ...
        self.findings = detect_all(source_files)
        self.diagnostics.add("info", "detector", "Detectors completed", findings=len(self.findings), rules=rule_ids())
        self.diagnostics.add("info", "orchestrator", "Audit completed successfully", files=[str(path) for path in source_files], compiler_version=version, is_poc=self.config.generate_poc, mapping_keys=list(mapping), findings=len(self.findings))
        return [{"file": str(path), "compiler_version": version, "mapped": path.name in mapping, "findings": sum(1 for finding in self.findings if str(path) in finding.sources)} for path in source_files]
    ...
    def _prepare_sources(self) -> list[Path]:
        ...
        if self.config.directory:
            ...
            if directory.is_dir():
                selected.extend(sorted(directory.rglob("*.sol")))
